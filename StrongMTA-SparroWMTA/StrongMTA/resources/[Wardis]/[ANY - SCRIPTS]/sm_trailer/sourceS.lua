...
addEventHandler("onElementClicked", getRootElement(),
	function(sourceKey, keyState, clickedPlayer)
		local playerCar = getPedOccupiedVehicle(clickedPlayer)
		if playerCar then
			if getElementType(source) == "vehicle" and getElementModel(source) == 611 then
				attachTrailerToVehicle(playerCar, source)
			end
		end
	end
)