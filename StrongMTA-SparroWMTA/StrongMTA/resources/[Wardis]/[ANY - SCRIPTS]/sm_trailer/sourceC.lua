...
local impoundColshape = createColCuboid(ZONE_MIN_X, ZONE_MIN_Y, ZONE_Z, ZONE_MAX_X - ZONE_MIN_X, ZONE_MAX_Y - ZONE_MIN_Y, 5)
local impoundMaterial = dxCreateTexture("files/road.png")
	...
	function (dataName, oldVal)
		if source == localPlayer then
			if dataName == "player.groups" then
				havePermission.impoundTow = exports.lv_groups:isPlayerHavePermission(localPlayer, "impoundTow")
				havePermission.impoundTowFinal = exports.lv_groups:isPlayerHavePermission(localPlayer, "impoundTowFinal")
			end
		end
		...
		if dataName == "carAnimation" then
			...
			if dataVal then
				...
				local rampX, rampY, rampZ = 0, 0, 0
				local targetPosX, targetPosY, targetPosZ = getPositionFromElementOffset(carAnimation[source][1], 0, -6, 0.1)
				...
				if carAnimation[source][2] == "up" then
					...
					local flatbedX, flatbedY, flatbedZ = getPositionFromElementOffset(effect.towedBy, 0, - 0.75, 0)
				...
				if carAnimation[source][2] == "down" then
					...
					local groundX, groundY, groundPreZ = getPositionFromElementOffset(effect.towedBy, 0, -8, minZ + 1)
...
function findAttachOffsets(vehicle, towedBy)
	...
	return 0, - 0.75, 0 + dist - 0.1, 0, 0, frRotZ - toRotZ
end
	...
	function ()
						...
						if isElementOnScreen(veh) or isElementOnScreen(holder) then
							...
							if dist <= 100 then
								...
								if holdertype == "vehicle" then
									...
									if side then
										local wx, wy, wz = getPositionFromElementOffset(veh, 0, 2, 0.1)
										...
										if cx and cy and cz then
											if cz < wz then
												local x1, y1, z1 = getPositionFromElementOffset(veh, 0, -1.15, 0.1)
												local x2, y2, z2 = getPositionFromElementOffset(veh, 0, -3.7, -0.15)
								...
								elseif holdertype == "player" then
									...
									-- Vontató kábel
									local x, y, z = getPositionFromElementOffset(veh, 0, 2, 0.1)
											...
											if tx then
												--[[if i == 4 then
													if holder == localPlayer then
														exports.lv_hud:showInfobox("w", "Itt nem tud átmenni a kábel!")
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				exports.lv_hud:showInfobox("w", "Ilyen messzire nem ér el a kábel!")
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								exports.lv_hud:showInfobox("e", "Előbb rakd a helyére az autószállító rámpáját!")
							end
						end
					end
				end
	...
	function (button, state, absX, absY, worldX, worldY, worldZ, clickedElement)
		if button == "right" then
			if state == "up" then
				if isElement(clickedElement) then
					if clickedElement == unImpoundPed then
						local px, py, pz = getElementPosition(localPlayer)
						...
						if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 3 then
							if not panelState then
								triggerServerEvent("openUnImpoundPanel", localPlayer)
							end
						end
					end
				end
		...
		if panelState then
			if activeButton then
				if button == "left" then
					if state == "up" then
						if gettok(activeButton, 1, "_") == "unImpound" then
							triggerServerEvent("unImpoundVehicle", localPlayer, impoundedVehicles[tonumber(gettok(activeButton, 2, "_"))].vehicleId)
						end
						...
						if not rampInAnim[veh] then
							...
							elseif key == "cardisc" then
								...
								if currentHolder then
									...
									if havePermission.impoundTowFinal then
										if isElement(currentHolder) and getElementType(currentHolder) == "vehicle" then
											local tx, ty, tz = getPositionFromElementOffset(veh, 0, -8, 0)

											if isInsideColShape(impoundColshape, tx, ty, tz) then
												triggerServerEvent("impoundTheVehicle", localPlayer, currentHolder)
											end
										end
									end
								end
								...
								if getElementType(clickedElement) == "vehicle" then
									if getVehicleType(clickedElement) == "Automobile" then
										...
										--if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											local x, y, z = getPositionFromElementOffset(myTowTruck, 0, -8, 0)
											local dist = getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)

											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												local groupId = getElementData(clickedElement, "vehicle.group") or 0

												if groupId == 0 then
													local jobVehID = getElementData(clickedElement, "jobVehicleID") or 0
													...
													if jobVehID == 0 then
														...
														outputChatBox("#d75959[SeeMTA - Lefoglalás]: #ffffffMunkajárműveket nem foglalhatsz le.", 255, 255, 255, true)
													end
												else
													outputChatBox("#d75959[SeeMTA - Lefoglalás]: #ffffffFrakciójárműveket nem foglalhatsz le.", 255, 255, 255, true)
												end
											else
												outputChatBox("#d75959[SeeMTA - Lefoglalás]: #ffffffAz autó nincs az autószállító mögött!", 255, 255, 255, true)
											end
										--else
											--outputChatBox("#d75959[SeeMTA - Lefoglalás]: #ffffffTúl messze vagy a járműtől!", 255, 255, 255, true)
										--end
									else
										outputChatBox("#d75959[SeeMTA - Lefoglalás]: #ffffffCsak autókat foglalhatsz le.", 255, 255, 255, true)
									end
								end
							end
						end
	...
	function (vehs)
		if vehs and type(vehs) == "table" then
			...
			for k, v in pairs(vehs) do
				...
				impoundedVehicles[k].realModelName = exports.lv_vehiclenames:getCustomVehicleName(v.modelId)
			end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		if panelState then
			local sx, sy = respc(500), respc(400)
			local x = screenX / 2 - sx / 2
			local y = screenY / 2 - sy / 2
			local buttons = {}

			dxDrawRectangle(x, y, sx, sy, tocolor(0, 0, 0, 150))
			dxDrawRectangle(x, y, sx, respc(30), tocolor(0, 0, 0, 200))
			dxDrawText("See#7cc576MTA #ffffff- Lefoglalt járművek", x + respc(7.5), y, 0, y + respc(30), tocolor(255, 255, 255), 1, panelFont, "left", "center", false, false, false, true)

			local tabNum = 13
			local oneSize = (sy - respc(40)) / tabNum

			buttons["close"] = {x + sx - respc(110), y + respc(5), respc(100), oneSize - respc(10)}

			if activeButton == "close" then
				dxDrawRectangle(x + sx - respc(110), y + respc(5), respc(100), oneSize - respc(10), tocolor(215, 89, 89, 225))
			else
				dxDrawRectangle(x + sx - respc(110), y + respc(5), respc(100), oneSize - respc(10), tocolor(215, 89, 89, 175))
			end

			dxDrawText("Bezárás", x + sx - respc(110), y + respc(5), x + sx - respc(10), y - respc(5) + oneSize, tocolor(0, 0, 0), 0.8, panelFont, "center", "center")

			local startY = y + respc(35)

			dxDrawRectangle(x + respc(5), startY, sx - respc(10), oneSize, tocolor(0, 0, 0, 225))
			dxDrawText("Jármű neve", x + respc(10), startY, 0, startY + oneSize, tocolor(124, 197, 118), 0.8, panelFont, "left", "center")
			dxDrawText("Eredeti jármű", x + respc(260), startY, 0, startY + oneSize, tocolor(124, 197, 118), 0.8, panelFont, "left", "center")

			tabNum = tabNum - 1

			for i = 1, tabNum do
				local y = startY + oneSize * i

				if i % 2 ~= 0 then
					dxDrawRectangle(x + respc(5), y, respc(490), oneSize, tocolor(0, 0, 0, 150))
				else
					dxDrawRectangle(x + respc(5), y, respc(490), oneSize, tocolor(0, 0, 0, 200))
				end

				local veh = impoundedVehicles[i + offsetVeh]

				if veh then
					dxDrawText(veh.realModelName, x + respc(10), y, 0, y + oneSize, tocolor(255, 255, 255), 0.8, panelFont, "left", "center")
					dxDrawText(veh.gtaModelName, x + respc(260), y, 0, y + oneSize, tocolor(255, 255, 255), 0.8, panelFont, "left", "center")

					buttons["unImpound_" .. i + offsetVeh] = {x + sx - respc(115), y + respc(5), respc(100), oneSize - respc(10)}

					if activeButton == "unImpound_" .. i + offsetVeh then
						dxDrawRectangle(x + sx - respc(115), y + respc(5), respc(100), oneSize - respc(10), tocolor(124, 197, 118, 225))
					else
						dxDrawRectangle(x + sx - respc(115), y + respc(5), respc(100), oneSize - respc(10), tocolor(124, 197, 118, 175))
					end

					dxDrawText("Kiváltás", x + sx - respc(115), y + respc(5), x + sx - respc(15), y + respc(5) + oneSize - respc(10), tocolor(0, 0, 0), 0.8, panelFont, "center", "center")
				end
			end

			if #impoundedVehicles > tabNum then
				local trackSize = tabNum * oneSize
				local startY = startY + oneSize

				dxDrawRectangle(x + sx - respc(5), startY, 5, trackSize, tocolor(0, 0, 0, 200))
				dxDrawRectangle(x + sx - respc(5), startY + offsetVeh * (trackSize / #impoundedVehicles), 5, trackSize / #impoundedVehicles * tabNum, tocolor(124, 197, 118, 150))
			end
		...
		dxDrawMaterialLine3D(
			ZONE_MIN_X + (ZONE_MAX_X - ZONE_MIN_X) / 2,
			ZONE_MAX_Y,
			ZONE_Z,
			ZONE_MIN_X + (ZONE_MAX_X - ZONE_MIN_X) / 2,
			ZONE_MIN_Y,
		...
		if havePermission.impoundTow then
			...
			if isElement(detectorColShape) then
				...
				if isElement(theTruck) and getVehicleSpeed(theTruck) < 40 then
					local tx, ty, tz = getPositionFromElementOffset(theTruck, 0, -8, 0)
					...
					if isElement(holder) and getElementType(holder) == "vehicle" then
						if havePermission.impoundTowFinal then
							...
							if sx and sy then
								...
								if isInsideColShape(impoundColshape, tx, ty, tz) then
									dxDrawImage(math.floor(sx - oneSize / 2), math.floor(sy - oneSize / 2), oneSize, oneSize, "files/impound.png", 0, 0, 0, tocolor(124, 197, 118))
								else
									dxDrawImage(math.floor(sx - oneSize / 2), math.floor(sy - oneSize / 2), oneSize, oneSize, "files/impound.png", 0, 0, 0, tocolor(215, 89, 89))
								end
							end
						end
					else
				...
				for veh, state in pairs(rampState) do
					if isElement(veh) then
						if isElementOnScreen(veh) then
							-- Forgattyú
							local x, y, z = getPositionFromElementOffset(veh, 0, 1.9, 0.2)
							...
							if dist <= 2.35 then
								...
								if x and y then
									...
									if isElement(holder) and getElementType(holder) == "vehicle" then
										...
										dxDrawRectangle(x, y, oneSize, oneSize, tocolor(25, 25, 25, 245))
										...
										dxDrawRectangle(x, y, oneSize * 2, oneSize, tocolor(25, 25, 25, 245))
