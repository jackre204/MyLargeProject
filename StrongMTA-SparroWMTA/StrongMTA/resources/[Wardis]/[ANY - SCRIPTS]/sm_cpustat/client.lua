...
function coloring(value)
	...
	elseif value > 10 and value <= 30 then 
		local r,g,b = 255,100,0
		a = tocolor(r,g,b);
	elseif value > 30 then 
		local r,g,b = 255,100,20
		a = tocolor(r,g,b);
	end
	return a;
end
		...
		for i, row in ipairs(clientRows) do
			if not disabledResources[row[1]] then
                ...
                if usedCPU > 0.1 then
                    ...
                    dxDrawText(text,x+1,y+1,150,15,tocolor(0,0,0,255),1,font, "left", "top", false, false, true)
                    dxDrawText(text,x,y,150,15,coloring(usedCPU),1,font, "left", "top", false, false, true)
                    y = y + 15
                end
			end
		end
