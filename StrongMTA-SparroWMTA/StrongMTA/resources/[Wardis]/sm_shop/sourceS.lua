	...
	function(pedId, items, totalPrice, totalSlot, totalWeight)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentMoney - totalPrice >= 0 then
				exports.sm_core:takeMoney(source, totalPrice, "shopBuyItems")
				...
				for k = 1, #items do
					...
					if nonStackableItems[itemId] then
						for i = 1, amount do
							exports.sm_items:addItem(source, itemId, 1)
						end
					else
						exports.sm_items:addItem(source, itemId, amount)
					end
				end

				--exports.crp_hud:showInfobox(source, "success", "Sikeres megvásároltad a kiválasztott termékeket!")

				dbExec(connection, "UPDATE peds SET balance = ? WHERE pedId = ?", pedsTable[pedId]["balance"], pedId)
			else
				--exports.crp_hud:showInfobox(source, "error", "Nincs nálad elegendő pénz!")
			end
		end
	end)
	...
	function(pedId, amount)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentBalance - amount >= 0 then
				exports.sm_core:giveMoney(source, amount, "shopGetOutMoney")
				...
				dbQuery(
					function(qh, player)
						if isElement(player) then
							...
							--exports.crp_hud:showInfobox(player, "success", "Sikeresen kifizettél a kasszából " .. amount .. " $-t.")
						end

						dbFree(qh)
					end, {source}, connection, "UPDATE peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
			else
				--exports.crp_hud:showInfobox(source, "error", "Nincs a kasszában ennyi pénz!")
			end
		end
	end)
	...
	function(pedId, amount)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentMoney - amount >= 0 then
				...
				exports.sm_core:takeMoney(source, amount, "shopPutInMoney")
				...
				dbQuery(
					function(qh, player)
						if isElement(player) then
							...
							--exports.crp_hud:showInfobox(player, "success", "Sikeresen befizettél a kasszába " .. amount .. " $-t.")
						end

						dbFree(qh)
					end, {source}, connection, "UPDATE peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
			else
				--exports.crp_hud:showInfobox(source, "error", "Nincs nálad ennyi pénz!")
			end
		end
	end)
		...
		if isElement(source) and pedId and pedsTable[pedId] then
			if pedsTable[pedId]["items"] and pedsTable[pedId]["items"][itemId] then
				...
				if currentBalance >= 0 then
					...
					exports.sm_core:takeMoney(source, amountPrice, "shopStock")
					...
					dbQuery(
						function(qh, player)
							if isElement(player) then
								...
								--exports.crp_hud:showInfobox(player, "success", "Sikeresen megrendeltél a kiválasztott tételből " .. amount .. " darabot " .. amountPrice .. " $ értékben.")
							end

							dbFree(qh)
						end, {source}, connection, "UPDATE peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
				else
					--exports.crp_hud:showInfobox(source, "error", "Nincs elegendő pénz a kasszában! (" .. amountPrice .. " $)")
				end
			end
		end
	end)
...
addCommandHandler("moveshopped",
	function(playerSource, commandName, pedId, skin)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not pedId then
				--outputChatBox(exports.crp_core:getServerTag("usage") .. "/" .. commandName .. " [Ped ID] [ < Skin ID > ]", playerSource, 0, 0, 0, true)
			else
				...
				if pedsTable[pedId] then
					...
					dbExec(connection, "UPDATE peds SET posX = ?, posY = ?, posZ = ?, rotZ = ?, interior = ?, dimension = ?, skinId = ? WHERE pedId = ?", x, y, z, rz, interior, dimension, skin, pedId)
				else
				--	outputChatBox(exports.crp_core:getServerTag("error") .. "A kiválasztott ped nem létezik!", playerSource, 0, 0, 0, true)
				end
			end
		end
	end)
...
addCommandHandler("delshopped",
	function(playerSource, commandName, pedId)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not pedId then
				--outputChatBox(exports.crp_core:getServerTag("usage") .. "/" .. commandName .. " [Ped ID]", playerSource, 0, 0, 0, true)
			else
				pedId = math.floor(math.abs(pedId))
				...
				if pedsTable[pedId] then
					dbQuery(
						function(qh, sourcePlayer, id)
							...
							--	outputChatBox(exports.crp_core:getServerTag("admin") .. "Sikeresen törölted a kiválasztott pedet.", playerSource, 0, 0, 0, true)
							end

							dbFree(qh)
						end, {playerSource, pedId}, connection, "DELETE FROM peds WHERE pedId = ?", pedId)
				else
					--outputChatBox(exports.crp_core:getServerTag("error") .. "A kiválasztott ped nem létezik!", playerSource, 0, 0, 0, true)
				end
			end
...
function savePeds(pedId)
	if tonumber(pedId) then
		dbExec(connection, "UPDATE peds SET itemList = ? WHERE pedId = ?", toString(pedsTable[pedId]), pedId)
	else
		for k, v in pairs(pedsTable) do
			dbExec(connection, "UPDATE peds SET itemList = ? WHERE pedId = ?", toString(v), k)
		end
	...
	function(res)
		...
		elseif source == getResourceRootElement() then
			...
			dbQuery(
				...
				end, connection, "SELECT * FROM peds")
...
addCommandHandler("createshopped",
	function(playerSource, commandName, skin)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not skin then
				--outputChatBox(exports.crp_core:getServerTag("usage") .. "/" .. commandName .. " [Skin ID]", playerSource, 0, 0, 0, true)
				--outputChatBox(exports.crp_core:getServerTag("info") .. "#ffff99A ped létrehozása után be kell állítanod a ped item kategóriáit.", playerSource, 0, 0, 0, true)
			else
				local x, y, z = getElementPosition(playerSource)
				local rx, ry, rz = getElementRotation(playerSource)
				local interior = getElementInterior(playerSource)
				...
				dbExec(connection, "INSERT INTO peds (posX,posY,posZ,rotZ,interior,dimension,skinId,itemList) VALUES (?,?,?,?,?,?,?,'')", x, y, z, rz, interior, dimension, skin)
				dbQuery(
					function(qh, sourcePlayer)
						...
						if result then
							...
							if isElement(sourcePlayer) then
								--outputChatBox(exports.crp_core:getServerTag("admin") .. "Sikeresen létrehoztál egy Bolt NPC-t. #32b3ef(PedId: " .. result["pedId"] .. ")", playerSource, 0, 0, 0, true)
							end
						end
					end, {playerSource}, connection, "SELECT * FROM peds WHERE pedId = LAST_INSERT_ID()")
			end
		end
	end)