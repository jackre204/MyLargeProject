...
addCommandHandler("nearbyshoppeds",
	function(commandName, maxdist)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not maxdist then
				outputChatBox(exports.crp_core:getServerTag("usage") .. "/" .. commandName .. " [ < távolság > ]", 0, 0, 0, true)
			else
				...
				if #peds > 0 then
					outputChatBox(exports.crp_core:getServerTag("admin") .. "Közeledben lévő pedek (" .. maxdist .. " yard):", 0, 0, 0, true)
					...
					outputChatBox(exports.crp_core:getServerTag("admin") .. "Nincs egyetlen ped sem a közeledben.", 0, 0, 0, true)
				end
			end
		end
	end)
...
addEventHandler("onClientResourceStart", getRootElement(),
	function(res)
		if getResourceName(res) == "sm_items" then
			itemNames = exports.sm_items:getItemNameList()
			nonStackableItems = exports.sm_items:getNonStackableItems()
		elseif source == getResourceRootElement() then
			local inventory = getResourceFromName("sm_items")
		...
		if isElement(element) and not currentPedId and button == "right" and state == "up" then
			...
			if pedId then
				...
				if isElementWithinColShape(localPlayer, colshape) then
					...
					RobotoFont = dxCreateFont("files/Roboto.ttf", respc(15), false, "antialiased")
					RobotoLighter = dxCreateFont("files/Roboto.ttf", respc(15), false, "antialiased")
...
function checkOwnership()
	...
	if tonumber(currentPedInterior) then
		if exports.sm_interiors:getInteriorOwner(currentPedInterior) == getElementData(localPlayer, "char.ID") or (getElementData(localPlayer, "acc.adminLevel") >= 6 and getElementData(localPlayer, "adminDuty")) then
			isOwner = true
		end
	end
...
function renderTheShop()
	...
	if promptDatas then
		...
		drawButton("acceptPrompt", "Igen", x + respc(20), y, sizeForButton, respc(40), {124, 197, 118}, 1, RobotoLighter, 1)

		drawButton("declinePrompt", "Nem", x + respc(40) + sizeForButton, y, sizeForButton, respc(40), 215, 89, 89, 1, RobotoLighter, 1)
	-- admin mód
	elseif panelState == "setcategories" then
		...
		--dxDrawImage(math.floor(x + logoOffset), math.floor(y + logoOffset), logoSize, logoSize, ":crp_hud/files/logo.png", 0, 0, 0, tocolor(50, 179, 239))
		...
		buttonsC["exit"] = {x + sx - respc(32) - logoOffset, y + titleSize / 2 - respc(16), respc(32), respc(32)}
		...
		for k = 1, maxVisible do
			...
			if activeButtonC == "setcat:" .. k + currentOffset then
			...
			if v then
				...
				buttonsC["setcat:" .. k + currentOffset] = {x2, y, sx2, sizeForRow}
			end
		...
		if haveChanges then
			...
			if activeButtonC == "savecats" then
			...
			buttonsC["savecats"] = {x, y, sx, titleSize}
		end
	-- tulajdonosi mód
	elseif panelState == "ownermode" then
		...
		if isOwner then
			drawButton("changeOwnerMode", "Vásárlói mód", buttonPosX, y + logoOffset, respc(230), logoSize, {61, 122, 188}, false, RobotoLighter, true)
		...
		if getElementData(localPlayer, "acc.adminLevel") >= 6 and getElementData(localPlayer, "adminDuty") then
			drawButton("adminmode", "Kategóriák beállítása", buttonPosX, y + logoOffset, respc(230), logoSize, {61, 122, 188}, false, RobotoLighter, true)
		end
		...
		for k = 1, maxVisible do
			...
			if v then
				if v[1] == "category" then
					...
					elseif activeButtonC == "togglecat:" .. k + currentOffset then
					...
					buttonsC["togglecat:" .. k + currentOffset] = {x2, y, sx2, sizeForRow}
				elseif v[1] == "noitem" and v[2] then
					...
					dxDrawImage(math.floor(x2 + respc(35)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":sm_items/files/items/" .. itemId .. ".png")
				...
				elseif v[1] == "haveitem" and v[2] and currentItems[v[2]] then
					...
					if activeButtonC == "togglesub:" .. k + currentOffset then
						colorOfRow = tocolor(53, 53, 53)
					end
					...
					if selectedActionItem == itemId then
						...
						drawButton("applyitemaction", "OK", buttonPosX, buttonPosY, respc(50), buttonSizeY, {61, 122, 188}, false, RobotoLighter, true)
						...
						drawButton("removeitem:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 200, 50, 50, 1, RobotoLighter, 0.8, "", IconsFont, 1)

						if currentPedInterior > 0 then
							buttonPosX = buttonPosX - respc(5) - respc(40)
							drawButton("addstock:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 7, 112, 196, 1, RobotoLighter, 0.8, "", IconsFont, 1)
						end

						buttonPosX = buttonPosX - respc(5) - respc(40)
						drawButton("setprice:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 7, 112, 196, 1, RobotoLighter, 0.8, "", IconsFont, 1)
		...
		if currentPedInterior > 0 then
			...
			if selectedItemAction == "putInMoney" then
				...
				drawButton("putInMoneyOk", "Befizetés", buttonPosX, y, respc(140), titleSize, 7, 112, 196, 1, RobotoLighter, 1)
			...
			elseif selectedItemAction == "getOutMoney" then
				...
				drawButton("getOutMoneyOk", "Kifizetés", buttonPosX, y, respc(140), titleSize, 200, 50, 50, 1, RobotoLighter, 1)
				...
				local buttonPosX = x + sx - respc(10) - respc(140)
				drawButton("getOutMoney", "Kifizetés", buttonPosX, y, respc(140), titleSize, 200, 50, 50, 1, RobotoLighter, 1)

				buttonPosX = buttonPosX - respc(10) - respc(140)
				drawButton("putInMoney", "Befizetés", buttonPosX, y, respc(140), titleSize, 7, 112, 196, 1, RobotoLighter, 1)
			end
		end
	-- vásárlói mód
	...
	--	dxDrawImage(math.floor(x + logoOffset), math.floor(y + logoOffset), logoSize, logoSize, ":crp_hud/files/logo.png", 0, 0, 0, tocolor(50, 179, 239))
		...
		if isOwner then
			if currentPedInterior > 0 then
				drawButton("changeOwnerMode", "Tulajdonosi mód", buttonPosX, y + logoOffset, respc(230), logoSize, {61, 122, 188}, false, RobotoLighter, true)
			else
				drawButton("changeOwnerMode", "Bolt kezelése", buttonPosX, y + logoOffset, respc(230), logoSize, {61, 122, 188}, false, RobotoLighter, true)
			end
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				if activeButtonC == "addtocart:" .. k + currentOffset then
					dxDrawRectangle(x2, y, sx2, sizeForRow, tocolor(53, 53, 53))
				end

				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":sm_items/files/items/" .. itemId .. ".png")
				...
				if currentPedInterior > 0 and (not itemData[2] or itemData[2] == 0) then
					...
					buttonsC["addtocart:" .. k + currentOffset] = {x2, y, sx2, sizeForRow}
				end
			end
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				if activeButtonC == "removefromcart:" .. k + cartOffset then
					dxDrawRectangle(x2, y2, sx2, sizeForRow, tocolor(53, 53, 53))
				end

				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y2 + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":sm_items/files/items/" .. itemId .. ".png")
				...
				buttonsC["removefromcart:" .. k + cartOffset] = {x2, y2, sx2, sizeForRow}
			end
		...
		if #cartItems > 0 then
			...
			drawButton("buyItems", "Fizetés", buttonPosX, y3, respc(140), titleSize - respc(5), {61, 122, 188}, false, RobotoLighter, true)
		end
	end
...
function shopClickHandler(button, state)
	...
	if currentPedId and button == "left" and state == "up" and activeButtonC then
		local selected = split(activeButtonC, ":")
		...
		elseif selected[1] == "buyItems" then
			if #cartItems > 0 then
				...
				for k = 1, #cartItems do
					...
					totalWeight = totalWeight + amount * exports.sm_items:getItemWeight(itemId)
				...
				if exports.sm_core:getMoney(localPlayer) >= totalPrice then
					if exports.sm_items:countEmptySlots() >= totalSlot then
						if exports.sm_items:getCurrentWeight() + totalWeight <= exports.sm_items:getWeightLimit("player", localPlayer) then
							...
							exports.crp_hud:showInfobox("error", "Nem bírsz el ennyi tárgyat!")
						end
					else
						exports.crp_hud:showInfobox("error", "Nincs elég hely az inventorydban!")
					end
				else
					exports.crp_hud:showInfobox("error", "Nincs nálad elegendő pénz!")
				end
			end
		end
	end
