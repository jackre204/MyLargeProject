...
size = Vector2(respc(200),respc(500))

sizes = {
    ["top"] = Vector2(respc(452),respc(62)),
    ["center"] = Vector2(respc(456),respc(33)),
    ["bottom"] = Vector2(respc(452),respc(40)),
    ["search"] = Vector2(respc(230),respc(20)),
}
a = "files/"

minLines = 1
maxLines = math.floor((screen.y-respc(150)-(sizes["top"].y+sizes["bottom"].y+respc(110)))/35)
...
function startScore()
    if getElementData(localPlayer, "loggedIn") then 
        ...
        scrolling = false
        state = true
        slots = getResourceRootElement(getResourceFromName("sm_core")):getData("server.maxPlayers")
        multipler = 20
        alpha = 0
        cacheCreate()
        if not _state then
...
local logoImage = svgCreate(respc(34),respc(34), logoRaw)
...
function drawnScoreboard()
    ...
    dxDrawRectangle(center.x-sizes["top"].x/2-2,y-2,respc(452+4),fullY+2,tocolor(25,25,25))
    dxDrawRectangle(center.x-sizes["top"].x/2,y,respc(452),respc(32),tocolor(35,35,35))
    local n = Vector4(center.x-sizes["top"].x/2,y,respc(452),respc(32))
    dxDrawImage(math.floor(n.x-respc(5)),math.floor(n.y-respc(2)),respc(34),respc(34),logoImage)
    dxDrawText("#3d7abcStrong#ffffffMTA",n.x+respc(35),n.y-2+n.w/2,n.x+respc(35),n.y-2+n.w/2,tocolor(200,200,200),1,RalewayC,"left","center",false,false,false,true)
    local gW,gH = respc(110),respc(18)
    local gX,gY = center.x+sizes["top"].x/2-respc(10)-gW,y+respc(32)/2-gH/2
    ...
    if not bar then
        CreateNewBar("search",{gX,gY,gW,gH},{25,"",false,nil,RalewayS,1,"center","center"},1)
        bar = true
    else
        UpdatePos("search",{gX,gY,gW,gH,alpha})
    end
    dxDrawRectangle(gX,gY,gW,gH,tocolor(200,200,200,40))
    local gColor = "#3d7abc"
    local tx = center.x-sizes["top"].x/2+respc(16)
    dxDrawText(gColor.."ID",tx,y+respc(32),tx,y+respc(32)+(sizes["top"].y-respc(32)),tocolor(200,200,200,200),1,Roboto,"center","center",false,false,false,true)
    local tx = center.x-sizes["top"].x/2+respc(65)
    local tx = center.x-sizes["top"].x/2+respc(200)
    dxDrawText(gColor.."Név",tx,y+respc(32),tx,y+respc(32)+(sizes["top"].y-respc(32)),tocolor(200,200,200,200),1,Roboto,"center","center",false,false,false,true)
    local tx = center.x-sizes["top"].x/2+respc(350)
    dxDrawText(gColor.."Szint",tx,y+respc(32),tx,y+respc(32)+(sizes["top"].y-respc(32)),tocolor(200,200,200,200),1,Roboto,"center","center",false,false,false,true)
    local tx = center.x-sizes["top"].x/2+respc(430) -- ping
    dxDrawText(gColor.."Ping",tx,y+respc(32),tx,y+respc(32)+(sizes["top"].y-respc(32)),tocolor(200,200,200,200),1,Roboto,"center","center",false,false,false,true)
    y = y+sizes["top"].y
    local __StartY = y
    for i = minLines,maxLines do
        if #text > 0 and searchCache[i] or #text <= 0 and cache[i] then
            ...
            if i == 1 then
                dxDrawRectangle(center.x-sizes["center"].x/2,y,sizes["center"].x-respc(7),sizes["center"].y,tocolor(61, 122, 188,200))
            end
            if i%2 == 0 then
                dxDrawRectangle(center.x-sizes["center"].x/2,y,sizes["center"].x-respc(7),sizes["center"].y,tocolor(35,35,35,200))
            else
                dxDrawRectangle(center.x-sizes["center"].x/2,y,sizes["center"].x-respc(7),sizes["center"].y,tocolor(45,45,45,200))
            end
            dxDrawRectangle(center.x-sizes["center"].x/2,y+sizes["center"].y,sizes["center"].x-respc(7),respc(2),tocolor(200,200,200,50))
            local rx = center.x-sizes["top"].x/2+respc(16) 
            local rx = center.x-sizes["top"].x/2+respc(16) 
            dxDrawText(id,rx,y,rx,y+sizes["center"].y,tocolor(156,156,156,(loggedin and alpha) or math.min(255 * 0.25,alpha)),1,Roboto,"center","center",false,false,false,true)
            if loggedin then
                local rx = center.x-sizes["top"].x/2+respc(350)
                dxDrawText(lvl,rx,y,rx,y+sizes["center"].y,tocolor(156,156,156,alpha),1,Roboto,"center","center",false,false,false,true)
            end
            local rx = center.x-sizes["top"].x/2+respc(430)
            local n = 20
            if loggedin then
                dxDrawText(ping,rx,y,rx,y+sizes["center"].y,tocolor(pingColor[1],pingColor[2],pingColor[3],200),1,Roboto,"center","center",false,false,false,true)
            end
            if aduty == 1 then
            ...
            if not loggedin then
                local w = dxGetTextWidth(name,1,Roboto,true)
                local h = dxGetFontHeight(1,Roboto)
                local x,y = center.x-sizes["top"].x/2+respc(200)-w/2,y+sizes["center"].y/2-h/2
            end
            local rx = center.x-sizes["top"].x/2+respc(200) -- név
            dxDrawText(name,rx,y,rx,y+sizes["center"].y,tocolor(156,156,156,(loggedin and alpha) or math.min(255 * 0.25,alpha)),1,Roboto,"center","center",false,false,false,true)
    ...
    if percent >= 1  then
        local gW,gH = respc(5),y-__StartY
        ...
        dxDrawRectangle(gX,gY,gW,gH,tocolor(200,200,200,math.min(255 * 0.05,alpha)))
        ...
        dxDrawRectangle(gX+respc(1),gY+respc(1),gW-respc(2),gH-respc(2),tocolor(r,g,b,100))

    end
    local x,y,w,h = center.x-sizes["search"].x/2,y+sizes["bottom"].y/2-sizes["search"].y/2,sizes["search"].x,sizes["search"].y
    dxDrawRectangle(x-respc(4),y-respc(4),w+respc(8),h+respc(8),tocolor(45,45,45))
    dxDrawRectangle(x,y,w*gMultipler,h,tocolor(61,122,188,180))
    dxDrawText(#cache.."/"..slots,x,y ,x+w,y+h,tocolor(200,200,200,math.min(255 * 0.5,alpha)),1,RalewayS,"center","center")
end
...
function cacheGetDetails(v,a2)
    ...
    a["aduty"] = v:getData("adminDuty") or 0
    a["aColor"] = exports.sm_administration:getAdminLevelColor(v:getData("acc.adminLevel")) or "#ffffff"
    a["aTitle"] = exports.sm_administration:getPlayerAdminTitle(v) or "Ismeretlen"
    local name = exports.sm_administration:getPlayerAdminNick(v,true) or "Ismeretlen"
    name = string.gsub(name,"_"," ")
    name = string.gsub(name,"#" .. (6 and string.rep("%x",6) or "%x+"),"")
    a["name"] = string.gsub(name,"#%x%x%x%x%x%x","")
    playerMinutes = v:getData("char.playedMinutes") or 0
    a["lvl"] = exports.sm_core:getLevel(false,playerMinutes)
    a["ping"] = v.ping or -1
...
addEventHandler("onClientElementDataChange",root,
    function(dName,oValue,nValue)
        if state then
            ...
            elseif dName == "server.maxPlayers" then
                slots = nValue
            ...
            elseif dName == "adminDuty" or dName == "acc.adminNick" or dName == "char.Name" or dName == "acc.adminLevel" then
                local k = search(source)
                if source == localPlayer then
                end
                if k then
                    local value = source:getData(dName)
                    if dName == "adminDuty" then
                        cache[k]["aduty"] = value
                    end
                    local name = exports.sm_administration:getPlayerAdminNick(source,true) or "Ismeretlen"
                    name = string.gsub(name,"_"," ")
                    cache[k]["name"] = string.gsub(name,"#%x%x%x%x%x%x","")
                    cache[k]["aColor"] = exports.sm_administration:getAdminLevelColor(source:getData("acc.adminLevel")) or "#ffffff"
                    cache[k]["aTitle"] = exports.sm_administration:getPlayerAdminTitle(source) or "Ismeretlen"
                    searchCache = {}
...
function drawText(text,x,y,w,h,color,size,font,left)
    if left then
        dxDrawText(text,x+respc(20),y+h/2,x+respc(20),y+h/2,color,size,font,"left","center",false,false,false,true)
    else
