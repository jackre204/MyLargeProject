...
local currentBasketScroll = 0
local maxBasketScroll = 10
...
local logoWidth = 45 / 2
local logoHeight = 65 / 2
local logoCreate = svgCreate(logoHeight-4, logoHeight-4, logoRaw)
local cartCreate = svgCreate(logoHeight-4, logoHeight-4, "files/cart.svg")

addEventHandler("onClientResourceStart", getRootElement(),
	function (startedRes)
		if getResourceName(startedRes) == "sm_items" then
			itemNames = exports.sm_items:getItemNameList()
		elseif source == getResourceRootElement() then
			local sm_items = getResourceFromName("sm_items")

			if sm_items and getResourceState(sm_items) == "running" then
				itemNames = exports.sm_items:getItemNameList()
			end
		end
	end)
...
addCommandHandler("nearbypeds",
	function (commandName, distance)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if #nearby > 0 then
				outputChatBox("#7cc576[Ped]: #ffffffKözeledben lévő pedek (" .. distance .. " yard):", 255, 255, 255, true)
				
				for k, v in ipairs(nearby) do
					outputChatBox("    * #7cc576Azonosító: #ffffff" .. v[1] .. " | #7cc576Név: #ffffff" .. utf8.gsub(v[2], "_", " ") .. " | #7cc576Inti: #ffffff" .. v[3], 255, 255, 255, true)
				end
			else
						...
						if pedId then
							if isElementWithinColShape(localPlayer, getElementData(clickedElement, "pedColShapeElement")) then
								if getElementData(clickedElement, "pedType") == 10 then
									...
									if exports.sm_core:getLevel(nil, playedMinutes) < 10 then
										exports.sm_hud:showInfobox("e", "Minimum 10 szint kell ahhoz, hogy fegyvert vehess!")
										return
									end

									if getElementData(localPlayer, "license.gun") ~= 1 then
										exports.sm_hud:showInfobox("e", "Fegyverengedély kell ahhoz, hogy fegyvert vehess!")
										return
									end
								end
...
function loadFonts()
	...
	Roboto = exports.sm_core:loadFont("Raleway.ttf", respc(10), false, "antialiased")
...
function checkOwnership()
	...
	if pedOwnerId then
		if exports.sm_interiors:getInteriorOwner(pedOwnerId) == getElementData(localPlayer, "char.ID") then
			isOwner = true
		end
	end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		if promptState then
			...
			dxDrawRectangle(x, y, sx, sy, tocolor(35, 35, 35, 255))
			...
			if promptState == "getOutMoney" then
				...
				dxDrawText("Összeg: " .. formatNumber(tonumber(promptInputText) or 0) .. " $\nJelenlegi egyenleg: " .. formatNumber(currentBalance) .. " $\nVégeredmény: " .. formatNumber(currentBalance - (tonumber(promptInputText) or 0)) .. " $\n#7cc576Add meg az összeget a billentyűzeteddel!", x + 10, y + 10, x + sx - 10, y + sy - 10 - 35 - 10, tocolor(255, 255, 255), 0.75, RobotoL, "center", "center", false, false, false, true)
			
				buttons["getOutMoneyOk"] = {x + 75, y + sy - 45, sx - 150, 35}

				if activeButtonC == "getOutMoneyOk" then
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 200))
				else
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 150))
				end
			...
			elseif promptState == "putInMoney" then
				...
				if activeButtonC == "putInMoneyOk" then
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 200))
				else
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 150))
				end
			...
			elseif promptState == "buyItem" then
				...
				dxDrawText("" .. itemNames[promptItemId], x + 1, y + respc(30), x + sx + 1, y + 1, tocolor(200, 200, 200, 200), 1, RobotoL, "center", "center")
				dxDrawText("Mennyiség: " .. (promptInputText or "") .. " db\nEgységár: " .. formatNumber(itemPrices[promptItemId] or 0) .. " $\nÖsszesen: " .. formatNumber((itemPrices[promptItemId] or 0) * (promptInputText or 1)) .. " $\n#7cc576Add meg a mennyiséget a billentyűzeteddel!", x + 10, y + 30, x + sx - 10, y + sy - 10 - 35 - 10, tocolor(200, 200, 200, 200), 0.75, RobotoL, "center", "center", false, false, false, true)
				drawButton("buyItemOk", "Vásárlás", x + 2, y + sy - 37, sx - 4, 35, {61, 122, 188}, false, Roboto)
			...
			elseif promptState == "setprice" then
				...
				dxDrawText("Ár: " .. (promptInputText or "") .. " $\nÁrrés: " .. (promptInputText or 0) - itemBasePrices[promptItemId] .. " $\nNagyker ár: " .. itemBasePrices[promptItemId] .. " $\n#7cc576Add meg az árat a billentyűzeteddel!", x + 10, y + 10, x + sx - 10, y + sy - 10 - 35 - 10, tocolor(255, 255, 255), 0.75, RobotoL, "center", "center", false, false, false, true)
			
				buttons["setPriceOk"] = {x + 75, y + sy - 45, sx - 150, 35}

				if activeButtonC == "setPriceOk" then
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 200))
				else
					dxDrawRectangle(x + 75, y + sy - 45, sx - 150, 35, tocolor(124, 197, 118, 150))
				end
		...
		elseif currentPedId then
			...
			if not ownerMode then
				dxDrawRectangle(x, y - respc(30), sx, sy + respc(30), tocolor(35, 35, 35, 255))
				dxDrawText("Bolt", x + respc(25), y - 35, x, y, tocolor(200, 200, 200, 200), 1, RobotoL, "left", "center")
				dxDrawImage(x, y - logoHeight, logoWidth, logoHeight, logoCreate)
				
				if #itemsInBasket > 0 then
					dxDrawRectangle(x + sx + 2, y, sx, sy, tocolor(35, 35, 35, 255))
					dxDrawText("Kosár",x + sx + 6 + 40, y + respc(17), nil, nil, tocolor(200, 200, 200, 200), 1, RobotoL, "left", "center", false, false, false, true)
					dxDrawImage(math.floor(x + sx + 6) + respc(5), y + respc(5), logoWidth, logoHeight, cartCreate, 0, 0, 0, tocolor(200, 200, 200, 200))
					for k, v in ipairs(itemsInBasket) do
						if k <= maxBasketScroll and (k > currentBasketScroll) then
							if k % 2 == 1 then
								dxDrawRectangle(x + sx + 4, y + oneSize*((k)-currentBasketScroll), sx - 4, oneSize, tocolor(25, 25, 25, 255))
							else
								dxDrawRectangle(x + sx + 4, y + oneSize*((k)-currentBasketScroll), sx - 4, oneSize, tocolor(45, 45, 45, 255))
							end
							dxDrawImage(math.floor(x + sx + 6), y + oneSize*((k)-currentBasketScroll) + respc(2), 36, 36, ":sm_items/files/items/" .. v[1] - 1 .. ".png")
							dxDrawText("".. exports.sm_items:getItemName(tonumber(v[1])) .."\n Ár: #3d7abc".. itemPrices[tonumber(v[1])]*tonumber(v[2]) .." $ #c8c8c8Mennyiség: #3d7abc".. tonumber(v[2]) .. " db", x + sx + 6 + 40, y + oneSize*((k)-currentBasketScroll), nil, nil, tocolor(200, 200, 200, 200), 1, Roboto, "left", "top", false, false, false, true)
							--drawButton("removeItemFromBasket" .. tonumber(v[1]), "Eltávolítás", x + sx + 36 + 270, y + oneSize*((k - 1)-currentBasketScroll) + 9, tocolor(215, 89, 89, 150), tocolor(215, 89, 89, 200), 0.8, 0.65)
							drawButton("removeItemFromBasket" .. tonumber(v[1]), "Eltávolítás", x + sx + respc(36) + respc(290), y + oneSize*((k)-currentBasketScroll) + respc(3), respc(70), oneSize - 6, {215, 89, 89, 200}, false, Roboto)						
						end
					end
					--drawButton("buyFromBasket", "Vásárlás", x + sx + 36 + 120, y + sy - 30, tocolor(215, 89, 89, 150), tocolor(215, 89, 89, 200), 1, 0.8)
					drawButton("buyFromBasket", "Vásárlás", x + sx + 4, y + sy - respc(36), sx - respc(4), oneSize - 6, {61, 122, 188}, false, Roboto)
				...
				for i = 1, tabNum do
					...
					if itemId then
						...
						if i % 2 == 1 then
							dxDrawRectangle(x, itemY, sx, oneSize, tocolor(25, 25, 25, 255))
						else
							dxDrawRectangle(x, itemY, sx, oneSize, tocolor(45, 45, 45, 255))
						end

						dxDrawImage(math.floor(x + 2), math.floor(itemY + 2), 36, 36, ":sm_items/files/items/" .. itemId - 1 .. ".png")
						...
						drawButton("buyitem_" .. itemId, "Vásárlás", x + sx - 15 - 2.5 - dxGetTextWidth("Vásárlás", 0.8, RobotoL) + respc(4), itemY + 13 - dxGetFontHeight(0.8, RobotoL) / 2, respc(70), oneSize - 6, {61, 122, 188}, false, Roboto)
						
						--drawButton("buyitem_" .. itemId, "Vásárlás", x + sx - 15 - dxGetTextWidth("Vásárlás", 0.8, RobotoL) - 2.5, itemY + 20 - dxGetFontHeight(0.8, RobotoL) / 2, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200), 0.8, 0.65)
					end
				end
				...
				if #availableItems > tabNum then
					...
					-- Scrollbar ha kellene
					--dxDrawRectangle(x + sx - 5, y, 5, totalSize, tocolor(0, 0, 0, 200))

					--dxDrawRectangle(x + sx - 5, y + offsetItems * gripSize, 5, tabNum * gripSize, tocolor(124, 197, 118, 150))
				end
				drawButton("exit", "Kilépés", x + sx - dxGetTextWidth("Kilépés", 0.8, RobotoL) - respc(20), y - dxGetFontHeight(0.8, RobotoL) - 5, respc(70), oneSize - 6 - respc(10), {215, 89, 89, 150}, false, Roboto)
				...
				if isOwner then
					drawButton("changeOwnerMode", "Tulajdonosi mód", x + sx - dxGetTextWidth("Tulajdonosi mód", 0.8, RobotoL) - 2.5, y + sy + 5, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200), 0.8, 0.7)
				end
			else
				...
				for i = 1, tabNum do
					...
					if itemId then
						...
						dxDrawImage(math.floor(itemX), math.floor(itemY + oneSize / 2 - 18), 36, 36, ":sm_items/files/items/" .. itemId - 1 .. ".png")
						...
						itemX = drawButton("addstock_" .. itemId, "Rendelés", itemX + w1, itemY, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200))

						itemX = drawButton("setprice_" .. itemId, "Beárazás", itemX + 5, itemY, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200))
				...
				-- Előző oldal
				if activeButtonC == "setcategory_" .. selectedCategory - 1 then
					dxDrawRectangle(x, y, holderSize, 35, tocolor(124, 197, 118, 175))
				else
				...
				-- Következő oldal
				if activeButtonC == "setcategory_" .. selectedCategory + 1 then
					dxDrawRectangle(x + holderSize * 2, y, holderSize, 35, tocolor(124, 197, 118, 175))
				else
				...
				for i = 1, #itemsInCategory[selectedCategory] do
					...
					if itemId and not availableItemsKeyed[itemId] then
						...
						if itemCount > visibleItem and itemCount <= visibleItem + tabNum then
							...
							dxDrawImage(math.floor(x2), math.floor(itemY + oneSize / 2 - 18), 36, 36, ":sm_items/files/items/" .. itemId - 1 .. ".png")
							...
							drawButton("additem_" .. itemId, "Hozzáadás", x + sx - 15 - dxGetTextWidth("Hozzáadás", 0.8, RobotoL) - 2.5, itemY + oneSize / 2 - dxGetFontHeight(0.8, RobotoL) / 2, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200), 0.8, 0.65)
				...
				if itemCount > tabNum then
					...
					dxDrawRectangle(x + sx - 5, y + 40 + visibleItem * gripSize, 5, tabNum * gripSize, tocolor(124, 197, 118, 150))
				end
				...
				local buttonX = drawButton("putInMoney", "Befizetés", buttonX - 5 - w2 - w1, y + sy - 35 + 17.5 - h / 2, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200), 0.75)
				...
				-- Vásárlói mód
				if isOwner then
					drawButton("changeOwnerMode", "Vásárlói mód", x + sx - dxGetTextWidth("Vásárlói mód", 0.8, RobotoL) - 2.5, y + sy + 5, tocolor(124, 197, 118, 150), tocolor(124, 197, 118, 200), 0.8, 0.7)
				end
			end
			...
			if button == "left" then
				if state == "up" then
					if activeButtonC then
						local selected = split(activeButtonC, "_")

						if activeButtonC == "exit" then
							closePanel()
						end

						if activeButtonC == "changeOwnerMode" then
							if checkOwnership() then
						...
						if selected[1] == "buyitem" then
							if maxItems[tonumber(selected[2])] == 1 then
								if not hasBasketItem({tonumber(selected[2]), 1}) then
									table.insert(itemsInBasket, {selected[2], 1})
								end
							else
								promptState = "buyItem"
								promptItemId = tonumber(selected[2])
						...
						elseif selected[1] == "buyItemOk" then
							...
							if tonumber(promptInputText) then
								if tonumber(promptInputText) < 1 then
									exports.sm_accounts:showInfo("e", "Minimum 1 darabot vehetsz egyszerre!")
									return
								end

								if maxItems[promptItemId] and tonumber(promptInputText) > maxItems[promptItemId] then
									exports.sm_accounts:showInfo("e", "Ebből maximum " .. maxItems[promptItemId] .. " darabot vehetsz egyszerre!")
									return
								end
								if not hasBasketItem({tonumber(promptItemId), tonumber(promptInputText)}) then
									table.insert(itemsInBasket, {promptItemId, tonumber(promptInputText)})
								end
						...
						for k, v in ipairs(itemsInBasket) do
							if selected[1] == "removeItemFromBasket"..v[1].."" then
								findBasketItem(tonumber(v[1]))
							end
						end
						if selected[1] == "buyFromBasket" then
							for k, v in ipairs(itemsInBasket) do
								setTimer(function()
									triggerServerEvent("buyItemFromPed", localPlayer, currentPedId, tonumber(v[1]), tonumber(v[2]), tonumber(v[2]) * tonumber(itemPrices[tonumber(v[1])]))
								end, k*100, 1)
								if k == #itemsInBasket then
									itemsInBasket = {}
								end
							end
						end
...
addEventHandler("onClientKey", getRootElement(), function(button, press)
    if currentPedId then
		if button == "mouse_wheel_up" then
			if currentBasketScroll > 0  then
				currentBasketScroll = currentBasketScroll -1
				maxBasketScroll = maxBasketScroll -1
			end
		elseif button == "mouse_wheel_down" then
			if maxBasketScroll < #itemsInBasket then
				currentBasketScroll = currentBasketScroll +1
				maxBasketScroll = maxBasketScroll +1
			end
		end
...
function hasBasketItem(table)
	if #itemsInBasket > 0 then
		for k, v in ipairs(itemsInBasket) do
			if tonumber(v[1]) == tonumber(table[1]) then
				hasBasket = true
				oldCount = itemsInBasket[k][2]
				itemsInBasket[k] = {tonumber(v[1]), tonumber(oldCount) + tonumber(table[2])}
				return true
			end
		end
	else
