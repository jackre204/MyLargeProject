...
local registrationPed = createPed(299, 1475.5855712891, -1802.5690917969, 18.7421875, 0)
setElementData(registrationPed, "invulnerable", true)
setElementData(registrationPed, "visibleName", "Christianus Alwin", true)
						...
						if pedType == "v3Registration" then
							...
							if getDistanceBetweenPoints3D(targetX, targetY, targetZ, playerX, playerY, playerZ) < 2.5 then
								if not panelState then
									...
									addEventHandler("onClientKey", getRootElement(), registrationKey)
								end
							end
						end
					end
...
local logoImage = svgCreate(respc(35) - respc(4), respc(35) - respc(4), logoRaw)


local fishingZones = {
	"Red Counry és környékei",
	"Los Santos",
	"Tierra Robada"
}

function registrationKey(key, state)
    if key == "backspace" and not state then
        ...
        removeEventHandler("onClientKey", getRootElement(), registrationKey)
        panelState = false
        showCursor(false)
    end
end
			...
			if activeButton then
				if string.find(activeButton, "makeDocument_") then
					...
					if tonumber(selected[2]) == 1 then
						triggerServerEvent("giveDocument", localPlayer, 65)
					-- Üres adásvételi
					elseif tonumber(selected[2]) == 6 then
						triggerServerEvent("giveDocument", localPlayer, 69)
					-- Horgászengedély
					elseif tonumber(selected[2]) == 3 then
						triggerServerEvent("giveDocument", localPlayer, 66, false, selected[3])
					-- Útlevél
					else
						triggerServerEvent("giveDocument", localPlayer, 67)
					end
				end
...
function registrationRender()
	local sx, sy = respc(600), respc(350)
	local x, y = screenX / 2 - sx / 2, screenY / 2 - sy / 2

	dxDrawRectangle(x, y, sx, sy+respc(12), tocolor(25, 25, 25))
	dxDrawRectangle(x+respc(4), y+respc(4), sx-respc(8), respc(35) - respc(4), tocolor(35, 35, 35))
	dxDrawText("#3d7abcStrong#ffffffMTA", x + respc(4) + respc(35) - respc(4), y + (respc(35) - respc(4)) / 2, nil, nil, tocolor(200, 200, 200, 200), 0.8, Raleway, "left", "center", false, false, false, true)
	dxDrawImage(x, y + respc(4), respc(35) - respc(4), respc(35) - respc(4), logoImage)

	buttonsC = {}

	local oneSize = (sy - respc(30)) /6

	for i=1, 6 do
		local y = y + oneSize * (i - 1) + respc(30)+respc(8)

		if i % 2 ~= 1 then
			dxDrawRectangle(x+respc(4), y, sx-respc(8), oneSize, tocolor(35, 35, 35))
		else
			dxDrawRectangle(x+respc(4), y, sx-respc(8), oneSize, tocolor(45, 45, 45))
		end

		if i > 5 then
			dxDrawText("Üres adásvételi (50 $)", x + respc(15), y, 0, y + oneSize, tocolor(200, 200, 200, 200), 0.75, Raleway, "left", "center")
		elseif i == 1 then
			dxDrawText("Személyi igazolvány (ingyenes)", x + respc(15), y, 0, y + oneSize, tocolor(200, 200, 200, 200), 0.75, Raleway, "left", "center")
		elseif i >= 3 then
			dxDrawText("Horgászengedély: " .. fishingZones[i - 2] .. " (250 $)", x + respc(15), y, 0, y + oneSize, tocolor(200, 200, 200, 200), 0.75, Raleway, "left", "center")
		else
			dxDrawText("Útlevél (100 $)", x + respc(15), y, 0, y + oneSize, tocolor(200, 200, 200, 200), 0.75, Raleway, "left", "center")
		end

		local buttonAlpha = 150
		...
		if i > 5 then
			drawButton("makeDocument_" .. i, buttonText, x + sx - respc(15) - respc(150), y2, respc(150), buttonHeight, {61, 122, 188}, false, Raleway, true, 0.75)
		elseif i >= 3 then
			drawButton("makeDocument_" .. fishingZones[i - 2], buttonText, x + sx - respc(15) - respc(150), y2, respc(150), buttonHeight, {61, 122, 188}, false, Raleway, true, 0.75)
		else
			drawButton("makeDocument_" .. i, buttonText, x + sx - respc(15) - respc(150), y2, respc(150), buttonHeight, {61, 122, 188}, false, Raleway, true, 0.75)
		end
	end
