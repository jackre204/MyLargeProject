...
local RAMP_DEFAULT_POSITION = {0.78211838006973, -2.5830059051514 + 0.35, -0.085817605257034 - 0.34}
...
local unImpoundPed = createPed(280, 2492.2661132813, 2522.4255371094, 10.8203125, 90)
setElementData(unImpoundPed, "invulnerable", true)
setElementData(unImpoundPed, "visibleName", "Clyde Shelton")
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
	--	havePermission.impoundTow = exports.sm_groups:isPlayerHavePermission(localPlayer, "impoundTow")
		havePermission.impoundTow = true
		havePermission.impoundTowFinal = true

		--havePermission.impoundTowFinal = exports.sm_groups:isPlayerHavePermission(localPlayer, "impoundTowFinal")
			...
			if dataVal then
				...
				if carAnimation[source][2] == "down" then
					...
					table.insert(effect.parts, {
						...
						startPos = effect.parts[1].endPos,
						startRot = effect.parts[1].endRot,
						endPos = {groundX, groundY, groundZ + dist + 1.8},
						endRot = {towRotX, towRotY, effect.parts[1].endRot[3]},
						sound = "winch",
						type = "detach",
					})
	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														exports.sm_hud:showInfobox("w", "Itt nem tud átmenni a kábel!")
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				exports.sm_hud:showInfobox("w", "Ilyen messzire nem ér el a kábel!")
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								exports.sm_hud:showInfobox("e", "Előbb rakd a helyére az autószállító rámpáját!")
							end
						end
					end
				end
								...
								if currentHolder then
									...
									if havePermission.impoundTowFinal then
										if isElement(currentHolder) and getElementType(currentHolder) == "vehicle" then
											local tx, ty, tz = getPositionFromElementOffset(veh, 0, -13, 0)
	...
	function (vehs)
		if vehs and type(vehs) == "table" then
			...
			for k, v in pairs(vehs) do
				...
				impoundedVehicles[k].realModelName = exports.sm_vehiclenames:getCustomVehicleName(v.modelId)
			end
