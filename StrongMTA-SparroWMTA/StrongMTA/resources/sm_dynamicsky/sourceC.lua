...
addEventHandler("onClientPreRender", getRootElement(),
	function ()
		...
		if minute ~= timeHMS[2] then
			...
			minuteStartTickCount = now
			minuteEndTickCount = minuteStartTickCount + 1000 / gameSpeed
		end

		if minuteStartTickCount then
			local minFraction = math.unlerpclamped(minuteStartTickCount, now, minuteEndTickCount)
...
local lastDim = getElementDimension(localPlayer)

function renderSphere()
	if dynamicSkyEnabled then
		local currentDim = getElementDimension(localPlayer)

		if lastDim ~= currentDim then
			lastDim = currentDim
			setElementDimension(modelTable.object, currentDim)
		end
